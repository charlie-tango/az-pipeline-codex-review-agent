...
function buildCommentSignature({
  content,
  filePath,
  startLine,
  endLine,
...
function buildThreadsUrl(options: CliOptions, repositoryId: string): string {
  ...
  const orgUrl = resolveOrganizationUrl(organization);
...
async function fetchExistingCommentSignatures(
  options: CliOptions,
  repositoryId?: string,
  gitApi?: IGitApi,
): Promise<Set<string>> {
  ...
  try {
    const restSignatures = await fetchExistingCommentSignaturesViaRest(
      options,
      repositoryId,
    );
...
async function fetchExistingCommentSignaturesViaRest(
  ...
  const response = await fetch(url, {
    method: "GET",
    headers: {
      Authorization: `Basic ${buildAuthHeader(options.azureToken)}`,
